...
mv k3s.yaml ~/.kube/k3s-vps2-kuzznya-config
# sed -i "s/127.0.0.1/$MASTER_IP/" ~/.kube/k3s-vps2-kuzznya-config   # some strange problems on Mac
echo "Please check that 127.0.0.1 is replaced to $MASTER_IP in ~/.kube/k3s-vps2-kuzznya-config"
# shellcheck disable=SC2016
echo 'Add "export KUBECONFIG=$HOME/.kube/k3s-vps2-kuzznya-config" to .profile manually'
